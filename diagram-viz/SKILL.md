---
name: diagram-viz
description: 绘制图表和信息可视化。支持两种模式：(1) draw.io 图表（流程图、架构图、时序图、ER图、思维导图、网络拓扑图）—— 当用户说"画一张图"、"绘制流程图/架构图"、"drawio格式"、"用draw.io画"时使用；(2) HTML 信息图（Infographic）—— 当用户说"按我的风格"、"信息图"、"Infographic"、"HTML格式可视化"，或内容包含多阶段/多角色分层、信息密度高、需要语义色彩分组时使用。两种模式都要截图验证，一次通过。
---
...
拿到需求后按以下优先级依次判断，命中即停止：

**1. 用户明确指定 → 遵从**
- 说了"draw.io"、"drawio格式"、".drawio 文件" → draw.io
- 说了"按我的风格"、"信息图"、"Infographic"、"HTML格式" → HTML 信息图

**2. 对话上下文有风格先例 → 沿用**
- 本次对话中已出现过 HTML 信息图输出（浅色渐变容器 + 白卡 + 左边框色条目）→ 默认继续用 HTML 信息图
- 本次对话中已出现过 draw.io 图表 → 默认继续用 draw.io

**3. 内容结构特征判断**

| 内容特征 | 倾向模式 |
|----------|---------|
| 节点之间有明确连线关系（流程、时序、依赖、ER） | draw.io |
| 多个并列"阶段/角色/模块"，每个内部有列表说明 | HTML 信息图 |
| 需要精确表达拓扑结构（谁连接谁） | draw.io |
| 信息密度高、需要语义色彩分组、分层嵌套展示 | HTML 信息图 |
| 内容类似"机制说明"、"对比分析"、"架构概览" | HTML 信息图 |

**4. 仍不确定 → 主动问**

不要猜，直接问：
> "这张图用 draw.io（流程图/连线拓扑）还是 HTML 信息图（分层分色的可视化）？"
