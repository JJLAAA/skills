...
def _load_dotenv() -> dict:
    """从 .env 文件加载环境变量，返回键值对字典。

    查找顺序：当前工作目录 > 脚本所在目录 > 用户主目录
    """
    env_vars = {}
    search_paths = [
        Path.cwd() / ".env",
...
def resolve_api_key(cli_key: Optional[str] = None) -> str:
    ...
    优先级：CLI --api-key > 环境变量 > .env 文件 > 配置文件 > 报错退出。
    """
    # 1. CLI 参数
    if cli_key:
        return cli_key
    ...
    # 3. .env 文件
    dotenv = _load_dotenv()
    dotenv_key = dotenv.get("IKUN_API_KEY", "").strip()
    if dotenv_key:
        return dotenv_key
    ...
    print(
        ...
        f"  2. 在项目目录或主目录创建 .env 文件，内容: IKUN_API_KEY=sk-xxx\n"
        f"  3. 手动创建 {CONFIG_FILE}，内容: {{\"api_key\": \"sk-xxx\"}}\n"
        "  4. 设置环境变量 IKUN_API_KEY=sk-xxx\n"
        "  5. 使用 --api-key sk-xxx 命令行参数",
        file=sys.stderr,
    )
    sys.exit(1)
